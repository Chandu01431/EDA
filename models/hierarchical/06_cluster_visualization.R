# ============================================
# TASK 5.6: HIERARCHICAL CLUSTER VISUALIZATION (FIXED)
# ============================================
...
load_rds_safe <- function(path, default_val = NULL) {
  if (file.exists(path)) {
    return(readRDS(path))
  } else {
...
# Load data safely
train_data_scaled <- load_rds_safe("models/kmeans/train_data_scaled.rds")
train_df_original <- load_rds_safe("models/kmeans/train_df_original.rds")
hc_clusters <- load_rds_safe("models/hierarchical/hc_clusters_training.rds")

# Fallback if cluster labels are missing
if (is.null(hc_clusters) && !is.null(train_data_scaled)) {
  hc_model <- hclust(dist(train_data_scaled), method = "ward.D2")
...
png("models/hierarchical/hc_clusters_original.png", width = 10, height = 8, units = "in", res = 100)
...
png("models/hierarchical/hc_clusters_scaled.png", width = 10, height = 8, units = "in", res = 100)
...
png("models/hierarchical/hc_cluster_sizes.png", width = 10, height = 7, units = "in", res = 100)
...
cluster_profiles_hc <- load_rds_safe("models/hierarchical/cluster_profiles_hc_train.rds")

if (!is.null(cluster_profiles_hc)) {
  ...
  png("models/hierarchical/hc_profiles_bar.png", width = 10, height = 7, units = "in", res = 100)
  ...
  print("  ⚠️ Skipped: cluster_profiles_hc_train.rds not found.")
}
